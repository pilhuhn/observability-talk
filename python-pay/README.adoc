...
[shell]
----
podman manifest create quay.io/pilhuhn/kafka-relay:multi
podman build -f Dockerfile-kafka --platform linux/amd64,linux/arm64 --manifest quay.io/pilhuhn/kafka-relay:multi .
podman manifest push quay.io/pilhuhn/kafka-relay:multi
----